...
USER_PROMPT = """Your next action must be to inspect the `context` variable using a ```repl``` block (e.g., print(context)). Do not answer until you have read the context. Solve only the task stated in the context and follow it faithfully. Be concise.\n\nContinue using the REPL environment, which has the `context` variable, and query sub-LLMs by writing to ```repl``` tags when needed. You may include multiple ```repl``` blocks, but if you include any, you must stop your response immediately after the last one and wait for the next iteration to see the output. Do not provide FINAL(...) or FINAL_VAR(...) in the same response as any ```repl``` block. Your next action:"""
